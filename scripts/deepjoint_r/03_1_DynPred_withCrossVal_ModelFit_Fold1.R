  ...
  # Part 1-1 : Fit the joint model to data from the 1st fold using jm_consensus
  ...
  args_long <- list(fixed = sqrt_Ylong ~ pts_age_modif + bl_age + manuf, 
  ...
  args_jm_CVCS <- list(time_var = "pts_age_modif", 
                       ...
                       parallel = 'multicore',
                       functional_forms = ~ value(sqrt_Ylong) + slope(sqrt_Ylong))
  ########
